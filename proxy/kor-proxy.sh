...
sudo iptables -N REDSOCKS -t nat
sudo iptables -I OUTPUT -t nat -j REDSOCKS # required by process that eg. VMs or local wget without http_proxy set
sudo iptables -I PREROUTING -t nat -j REDSOCKS # required by eg. docker
...
#### opmtyalizcja ruch !!!
sudo iptables -A REDSOCKS -t nat -p tcp -m tcp --dport 80 -j DNAT --to-destination 10.217.247.236:911
# sudo iptables -A REDSOCKS -t nat -p tcp -m tcp --dport 443 -j DNAT --to-destination 127.0.0.1:1080
sudo iptables -A REDSOCKS -t nat -p tcp -m tcp -j DNAT --to-destination 127.0.0.1:1080
...
sudo iptables -I FORWARD -p udp -m udp --dport 53 -m conntrack --ctstate NEW -j ACCEPT
sudo iptables -I FORWARD -p tcp -m tcp --dport 911 -m conntrack --ctstate NEW -j ACCEPT
